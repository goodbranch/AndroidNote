...
* G1收集器
	...
	* 将整个堆内存分成若干个独立区域（Region），虽然保留新生代和老年代概念，但是她们之间不是物理隔离，他们都是一部分Region。

	* 跟踪维持Region垃圾堆信息，在后台维持一个优先列表，每次根据优先级的区域回收方式，保证G1在有限时间内尽可能回收高的回收效率。

	* 通过Remembered set记录在不同Region块的引用，避免全栈扫描。
